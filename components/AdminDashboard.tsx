...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  useEffect(() => {
    if (
      isAdmin &&
      (currentUser.next_payment_due || currentUser.trial_ends_at)
    ) {
      const dueDateStr =
        currentUser.companyStatus === "trial"
          ? currentUser.trial_ends_at
          : currentUser.next_payment_due;
      if (dueDateStr) {
  ...
  const [isEditingFullOrder, setIsEditingFullOrder] = useState<string | null>(
    null,
  ...
  // Status & View Modals
  const [managingStatusOrder, setManagingStatusOrder] = useState<Order | null>(
    null,
  );
  const [viewingOrder, setViewingOrder] = useState<Order | null>(null);
  const [viewingOrderTransactions, setViewingOrderTransactions] = useState<
    any[]
  >([]);
  ...
  // Edit Payment State for View Modal
  const [editingPayment, setEditingPayment] = useState<any | null>(null);
  const [editPayAmount, setEditPayAmount] = useState("");
  const [editPayMethod, setEditPayMethod] = useState("Pix");
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesDate =
      (!dateStart || orderDateInput >= dateStart) &&
  ...
  const handleConvertQuote = async (id: string) => {
    if (
      !window.confirm(
        "Deseja converter este orçamento em um pedido oficial?\nIsso moverá o item para a aba de Pedidos e iniciará a linha do tempo.",
      )
    )
      return;
  ...
  const handleDuplicateOrder = async (order: Order) => {
    ...
    if (!order.photos || order.photos.length === 0) {
      try {
        ...
        console.error(
          "Erro ao buscar detalhes do pedido para duplicação:",
          err,
        );
      }
    }
  ...
  const handleSubmitOrder = async () => {
    ...
    if (
      ...
      alert(
        "Por favor, preencha os campos obrigatórios: Nome, Contato, Data e adicione pelo menos um produto.",
      );
      return;
    }
    ...
    try {
      ...
      alert(
        "Erro ao salvar. Se estiver enviando muitas fotos, tente enviar menos ou em menor resolução.\n\nDetalhes: " +
          err.message,
      );
    } finally {
      setIsSubmitting(false);
  ...
  const handleSubmitEmployee = async () => {
    ...
    if (
      ...
      !newEmployeeForm.role ||
      !newEmployeeForm.login ||
  ...
  const handleStatusChange = async (
    ...
    await updateOrderStatus(
      orderId,
      newStatus,
      updateStatusLocation || undefined,
    );
  ...
  const handleRegisterPayment = async () => {
    ...
    const isGift = !!viewingOrder.isGift || viewingOrder.paymentMethod === "Brinde / Patrocínio" || (viewingOrder.paymentMethod && viewingOrder.paymentMethod.toLowerCase().includes('brinde'));
    if (isGift) {
      alert("Este pedido é um brinde/patrocínio e não possui cobrança nem lançamento de valores.");
      return;
    }
    ...
    try {
      setLoading(true);
      const updatedOrder = await registerPayment(
        viewingOrder.id,
        amount,
  ...
  const handleCancelOrder = async (orderId: string) => {
    if (
      !window.confirm(
        "Deseja realmente cancelar este pedido? O status será alterado para Cancelado.",
      )
    )
      return;
    try {
      ...
      // If viewing this order, update local state
      if (viewingOrder && viewingOrder.id === orderId) {
        const updatedOrder = updatedList.find((o) => o.id === orderId) || null;
        setViewingOrder(updatedOrder);
      }
  ...
  const handleStartEditPayment = (payment: any) => {
    ...
    setEditPayMethod(payment.paymentMethod || "Pix");
    // Format date back to yyyy-MM-dd if it's in dd/MM/yyyy format
    let formattedDate = payment.date;
    if (formattedDate && formattedDate.includes("/")) {
      const [d, m, y] = formattedDate.split("/");
      formattedDate = `${y}-${m}-${d}`;
    }
    setEditPayDate(formattedDate || new Date().toISOString().split("T")[0]);
    setEditPayAccountId(payment.accountId || payment.account_id || "");
  };

  const handleSavePayment = async () => {
    if (!editingPayment || !viewingOrder) return;
    if (!editPayAmount) {
      alert("Informe o valor do pagamento.");
      return;
    }
    if (!editPayAccountId) {
      alert("Selecione a conta para o pagamento.");
      return;
    }
    ...
    const remainingLimit = viewingOrder.total - (viewingOrder.downPayment || 0);

    if (difference > remainingLimit + 0.01) {
      alert("O novo valor excede o saldo restante do pedido.");
      return;
    }
    ...
    try {
      ...
      await updateTransaction(editingPayment.id, {
        ...
        amount: amount,
        date: editPayDate
          ? editPayDate.split("-").reverse().join("/")
          : new Date().toLocaleDateString("pt-BR"),
        paymentMethod: editPayMethod,
        accountId: editPayAccountId,
        orderId: viewingOrder.id,
      });

      // Refresh the order details and the transactions list
      const updatedList = await getAllOrders();
      setOrders(updatedList);
      const updatedOrder =
        updatedList.find((o) => o.id === viewingOrder.id) || null;
      setViewingOrder(updatedOrder);
      ...
      alert("Erro ao editar pagamento: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRemovePayment = async (transactionId: string) => {
    ...
    if (
      !window.confirm(
        "Deseja realmente remover este registro de pagamento? O saldo do pedido será recalculado e o saldo da conta bancária será estornado.",
      )
    )
      return;
  ...
  const handlePrintOrder = () => {
    ...
    const subtotal = viewingOrder.items.reduce(
      (acc, item) => acc + item.price * item.quantity,
      0,
    );
                ...
                <div style="font-size: 14px; color: #e53e3e;">
                    ...
                    - ${new Intl.NumberFormat("pt-BR", {
                      ...
                      viewingOrder.discountType === "percentage"
                        ? subtotal * (viewingOrder.discount / 100)
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-30 no-print">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <span
              className={`px-2 py-0.5 rounded-md text-xs font-semibold uppercase ${isAdmin ? "bg-purple-100 text-purple-700" : "bg-gray-100 text-gray-600"}`}
            >
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={onLogout}
              className="text-gray-500 hover:text-red-600 text-sm font-medium flex items-center gap-1"
            >
              <X size={16} /> Sair
            </button>
          </div>
        ...
        <div className="bg-orange-50 border-b border-orange-100 p-3 no-print">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
            <div className="flex items-center gap-2 text-orange-800 text-sm font-medium">
              ...
              Atenção: Sua assinatura vence em {daysRemaining} dia(s). Evite o
              bloqueio do sistema.
            </div>
            <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 no-print">
          ...
          {(activeTab === "orders" || activeTab === "quotes") && (
            ...
            >
              <Plus size={16} />{" "}
              {activeTab === "quotes" ? "Novo Orçamento" : "Novo Pedido"}
            </button>
          )}
        </div>
                ...
                <div className="flex items-center gap-4 w-full md:w-auto md:ml-4">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-600">
                    <input
                      ...
                      onChange={() => {
                        setDateFilterField("estimatedDelivery");
                        setCurrentPage(1);
                      }}
                      className="text-primary focus:ring-primary"
                    />
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentOrders.length === 0 ? (
                      <tr>
                        ...
                        >
                          Nenhum{" "}
                          {activeTab === "quotes" ? "orçamento" : "pedido"}{" "}
                          encontrado.
                        </td>
                      </tr>
                    ) : (
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            {activeTab === "quotes" ? (
                              <div className="text-xs text-purple-600 font-medium">
                                Validade: {order.quoteValidity || "-"}
                              </div>
                            ) : (
                              <div className="text-xs text-gray-900 font-medium">
                                Ent: {order.estimatedDelivery}
                              </div>
                            )}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col">
                          ...
                          <span
                            className={`text-xs font-semibold uppercase ${emp.accessLevel === "admin" ? "text-purple-600" : "text-gray-500"}`}
                          >
                            {emp.accessLevel}
                          </span>
                        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-6 border-b border-gray-100">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Personalize a identidade visual e dados da sua empresa.
                </p>
              </div>
              <div className="p-8 space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Endereço de Retirada / Loja Física
                  </label>
                  <input
                    type="text"
                    className="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-primary focus:outline-none"
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-blue-100">
                      Gerencie os detalhes do seu plano Rastreaê.
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <div>
                      ...
                      >
                        ...
                        {currentUser.companyStatus === "trial" &&
                          "Período de Teste"}
                        {currentUser.companyStatus === "pending_payment" &&
                          "Pagamento Pendente"}
                  ...
                  <div className="flex flex-col justify-center items-center bg-gray-50 rounded-xl border border-gray-200 p-6 text-center">
                    ...
                    <p className="text-sm text-gray-500 mb-6">
                      Realize o pagamento de forma segura via Mercado Pago e
                      libere o acesso instantaneamente.
                    </p>
                    <button
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              ...
              <button onClick={() => setShowNewEmployeeModal(false)}>
                  ...
                  <div className="flex gap-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        className="text-primary focus:ring-primary"
                      />
                      <span className="text-sm text-gray-700 font-semibold text-purple-700">
                        Administrador
                      </span>
                    </label>
                  </div>
                  <p className="text-xs text-gray-500 mt-2">
                    * Usuários não podem excluir pedidos nem gerenciar
                    funcionários.
                  </p>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="font-bold text-lg text-gray-800">
                {isEditingFullOrder
                  ? `Editar ${activeTab === "quotes" ? "Orçamento" : "Pedido"}`
                  : `Cadastrar Novo ${activeTab === "quotes" ? "Orçamento" : "Pedido"}`}
              </h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Celular / Contato <span className="text-red-500">*</span>
                  </label>
                  <div className="relative">
                    <Phone
                      size={18}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Data {orderForm.isQuote ? "do Orçamento" : "do Pedido"}{" "}
              ...
              <div className="border-t border-gray-100 pt-4">
                ...
                <div className="flex gap-4 items-start flex-wrap">
                  {orderForm.photos.map((photo, index) => (
                    <div
                      key={index}
                      className="relative w-20 h-20 rounded-lg border border-gray-300 overflow-hidden group"
                    >
                      <img
                        src={photo}
                        alt="Preview"
                  ...
                  {orderForm.photos.length < 6 && (
                    <label
                      className={`w-20 h-20 rounded-lg border-2 border-dashed border-gray-300 flex flex-col items-center justify-center cursor-pointer hover:border-primary hover:text-primary text-gray-400 transition ${isProcessingImage ? "opacity-50 cursor-wait" : ""}`}
                    >
                      {isProcessingImage ? (
                      ...
                      >
                        ...
                        {accounts
                          .filter(
                            (acc) =>
                              acc.active ||
                              acc.id === orderForm.downPaymentAccountId,
                          )
                          .map((acc) => (
              ...
              <div className="border-t border-gray-100 pt-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    {!tempItem.isSet && (
                      <input
                        ...
                        value={tempItem.size}
                        onChange={(e) =>
                          setTempItem({ ...tempItem, size: e.target.value })
                        }
                      />
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setTempItem({ ...tempItem, price: e.target.value })
                        }
                      />
                      <input
                        type="number"
                        ...
                        onChange={(e) =>
                          setTempItem({ ...tempItem, quantity: e.target.value })
                        }
                      />
                    </div>
                    ...
                    <label className="flex items-center gap-2 cursor-pointer bg-white px-3 py-2 rounded-lg border border-gray-200 shadow-sm hover:bg-gray-50 transition">
                      <input
                        ...
                        onChange={(e) =>
                          setTempItem({ ...tempItem, isSet: e.target.checked })
                        }
                      />
                ...
                <div className="bg-gray-100 rounded-xl p-4 space-y-3">
                  {orderForm.items.length === 0 && (
                    <p className="text-sm text-center text-gray-400 py-4 italic">
                  ...
                  {orderForm.items.map((item, idx) => (
                    <div
                      key={idx}
                      className={`relative flex justify-between items-start bg-white p-4 rounded-xl shadow-sm border ${editingItemIndex === idx ? "border-primary" : "border-gray-200"}`}
                    >
                      <div className="flex-1">
                          ...
                          item.subItems.length > 0 && (
                            <div className="mt-2 pl-2 border-l-2 border-purple-100 flex flex-wrap gap-2">
                              {item.subItems.map((sub, sIdx) => (
                                <span
                                  key={sIdx}
                                  className="text-[10px] bg-purple-50 text-purple-600 px-2 py-0.5 rounded"
                                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-gray-100 pt-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="flex gap-1 bg-gray-100 p-1 rounded-lg border border-gray-300 flex-shrink-0">
                      <button
                        type="button"
                        onClick={() =>
                          setOrderForm({ ...orderForm, discountType: "fixed" })
                        }
                ...
                <div>
                  ...
                  <div className="p-2.5 bg-blue-50 border border-blue-200 rounded-lg font-bold text-primary">
                    {(() => {
                      const sub = orderForm.items.reduce(
                        (acc, item) => acc + item.price * item.quantity,
                        0,
                      );
                      let tot = sub;
            ...
            <div className="p-4 bg-gray-50 flex justify-end gap-2 border-t border-gray-100">
              ...
              >
                {isSubmitting ? (
                  ...
                  `Atualizar ${orderForm.isQuote ? "Orçamento" : "Pedido"}`
                ) : (
                  `Criar ${orderForm.isQuote ? "Orçamento" : "Pedido"}`
                )}
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden">
            ...
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  #{managingStatusOrder.id}
                </p>
              </div>
              <button onClick={() => setManagingStatusOrder(null)}>
                <X className="text-gray-400 hover:text-gray-600" />
              </button>
            </div>
                ...
                {[
                  ...
                  >
                    <span className="font-medium text-gray-700">
                      ...
                      {status === OrderStatus.CANCELADO && "🚫 Cancelado"}
                    </span>
                    {status === managingStatusOrder.currentStatus && (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl w-full max-w-3xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    <>
                      {viewingOrder.pressingDate && (
                        <div className="flex justify-between md:justify-end gap-2 text-sm text-gray-700">
                          <span>Prensagem:</span>
                          <span className="font-medium">
                            {formatDatePTBR(viewingOrder.pressingDate)}
                          </span>
                        </div>
                      )}
                      {viewingOrder.printingDate && (
                        <div className="flex justify-between md:justify-end gap-2 text-sm text-gray-700">
                          <span>Impressão:</span>
                          <span className="font-medium">
                            {formatDatePTBR(viewingOrder.printingDate)}
                          </span>
                        </div>
                      )}
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {viewingOrder.items.map((item, idx) => (
                      <tr key={idx} className="even:bg-white odd:bg-gray-50/50">
                                ...
                                item.subItems.length > 0 && (
                                  <div className="mt-1 flex flex-wrap gap-1">
                                    {item.subItems.map((sub, sIdx) => (
                                      <span
                                        key={sIdx}
                                        className="text-[10px] bg-purple-50 text-purple-600 px-1.5 py-0.5 rounded border border-purple-100/50"
                                      >
                                        {sub.name}: {sub.size}
                                      </span>
                                    ))}
                        ...
                        <td className="px-4 py-3 text-sm text-center text-gray-600 font-bold">
              ...
              <div className="flex justify-end mb-6">
                <div className="w-full md:w-1/2 bg-white rounded-xl border border-gray-200 overflow-hidden">
                  ...
                  <table className="min-w-full divide-y divide-gray-100">
                    <tbody>
                      {Object.entries(
                        getSizeSummary(viewingOrder.items).summary,
                      ).map(([size, qty]) => (
                        <tr key={size}>
                      ...
                      <tr className="bg-gray-50 font-bold">
                        ...
                        <td className="px-4 py-2 text-sm text-gray-800 text-right">
                          {getSizeSummary(viewingOrder.items).totalItems}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 space-y-4">
                      ...
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <span className="font-medium text-green-600">
                          {new Intl.NumberFormat("pt-BR", {
                            style: "currency",
                            currency: "BRL",
                          }).format(viewingOrder.downPayment || 0)}
                        </span>
                      </div>
                      {(viewingOrder.isGift || viewingOrder.paymentMethod === "Brinde / Patrocínio" || (viewingOrder.paymentMethod && viewingOrder.paymentMethod.toLowerCase().includes('brinde'))) ? (
                        ...
                        <div className="border-t border-gray-200 pt-2 flex justify-between items-center">
                          ...
                          <span
                            className={`font-bold text-lg ${viewingOrder.total - viewingOrder.downPayment > 0.01 ? "text-red-600" : "text-green-600"}`}
                          >
                            {new Intl.NumberFormat("pt-BR", {
                              style: "currency",
                              currency: "BRL",
                              ...
                              Math.max(
                                0,
                                viewingOrder.total - viewingOrder.downPayment,
                              ),
                            )}
                          </span>
                        </div>
                              ...
                              <div className="flex flex-col">
                                ...
                                <div className="flex items-center gap-2 text-[10px] text-gray-400">
                                  ...
                                  <span className="text-blue-500">
                                    {t.accountName || "Sem conta"}
                                  </span>
                                </div>
                              </div>
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                <button
                                  onClick={() => handleStartEditPayment(t)}
                  ...
                  {(viewingOrder.isGift || viewingOrder.paymentMethod === "Brinde / Patrocínio" || (viewingOrder.paymentMethod && viewingOrder.paymentMethod.toLowerCase().includes('brinde'))) ? (
                    <div className="pt-4 border-t border-gray-200">
                      <div className="p-4 bg-gray-100 border border-gray-200 rounded-xl flex items-center justify-between gap-3 text-gray-500 select-none">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-lg bg-gray-200 text-gray-500 flex items-center justify-center font-bold">
                            <Lock size={18} />
                          </div>
                          <div>
                            <p className="text-xs font-bold text-gray-700 uppercase tracking-wider">
                              Lançamento de Pagamento Desativado
                            </p>
                            <p className="text-xs text-gray-500">
                              Pedido identificado como Brinde / Patrocínio (Sem cobrança). Não é permitido lançar valores.
                            </p>
                          </div>
                        </div>
                        <span className="px-3 py-1.5 bg-gray-200 text-gray-600 text-xs font-bold rounded-lg border border-gray-300">
                          Bloqueado (Brinde)
                        </span>
                      </div>
                    </div>
                        ...
                        <div className="space-y-3">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <select
                              ...
                              onChange={(e) =>
                                setPaymentMethodRemaining(e.target.value)
                              }
                            >
                              <option value="Pix">Pix</option>
                              <option value="Dinheiro">Dinheiro</option>
                            ...
                            >
                              ...
                              {financeAccounts
                                .filter(
                                  (acc) =>
                                    acc.active || acc.id === paymentAccountId,
                                )
                                .map((acc) => (
                                  <option key={acc.id} value={acc.id}>
            ...
            <div className="p-4 bg-gray-50 flex justify-end gap-2">
              {viewingOrder.currentStatus !== OrderStatus.CANCELADO &&
                viewingOrder.currentStatus !== OrderStatus.CONCLUIDO &&
                viewingOrder.currentStatus !== OrderStatus.ORCAMENTO && (
                  <button
                    onClick={() => handleCancelOrder(viewingOrder.id)}
                    className="px-6 py-2 bg-red-50 text-red-700 font-medium rounded-lg hover:bg-red-100 flex items-center gap-2 border border-red-200 transition"
                  >
              ...
              <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden border border-gray-100">
                <div className="p-4 bg-blue-50 border-b border-blue-100 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Ajustar detalhes do recebimento
                    </p>
                  </div>
                  <button
                ...
                <div className="p-6 space-y-4 text-left">
                  <div>
                    ...
                    <select
                      className="w-full border border-gray-200 rounded-lg p-2.5 text-sm bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all"
                      value={editPayMethod}
                  ...
                  <div>
                    ...
                    >
                      ...
                      {financeAccounts
                        .filter(
                          (acc) => acc.active || acc.id === editPayAccountId,
                        )
                        .map((acc) => (
                          <option key={acc.id} value={acc.id}>
                            {acc.name} {!acc.active && "(Inativa)"}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          type="number"
                          placeholder="Valor"
                          className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all"
                          value={editPayAmount}
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        className="w-full border border-gray-200 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all"
                        value={editPayDate}
                ...
                <div className="p-4 bg-gray-50 border-t border-gray-100 flex gap-2 justify-end">
                  ...
                  <button
                    ...
                    disabled={!editPayAmount || loading}
                    className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg text-sm font-bold transition disabled:opacity-50 flex items-center gap-2"
                  >
                    {loading && (
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                    )}
